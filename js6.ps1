...
### ✅ Post-validation: Check JSON validity
Write-Host "`nPost-validation Checks:"

if (-not (Test-Path $outputPath)) {
    Write-Host "❌ JSON file not found after generation!"
    exit 1
}

try {
    $json = Get-Content $outputPath -Raw | ConvertFrom-Json -ErrorAction Stop
    if ($null -eq $json.POOLS -or $json.POOLS.Count -eq 0) {
        Write-Host "❌ JSON structure is invalid or POOLS is empty!"
        exit 1
    } else {
        Write-Host "✅ JSON file is valid and contains $($json.POOLS.Count) pool entries."
    }
}
catch {
    Write-Host "❌ Failed to parse JSON file. Error: $_"
    exit 1
}
